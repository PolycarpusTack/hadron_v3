//! Crash analysis eval harness — web surface.
//!
//! Standalone binary that reads fixtures, calls the AI provider directly
//! with hadron-core's crash analysis prompt, and outputs CSV.
//!
//! Usage:
//!   SQLX_OFFLINE=true cargo run --bin eval -- --model gpt-4.1 --provider openai --api-key $KEY
//!
...
// Use the same prompt the web surface uses for crash analysis
const SYSTEM_PROMPT: &str = hadron_core::ai::prompts::CRASH_ANALYSIS_PROMPT;
...
async fn main() {
    ...
    eprintln!("[web eval] Loaded {} fixtures from {}", fixtures.len(), dir.display());
        ...
        let row = match response {
            Ok(text) => {
                match serde_json::from_str::<serde_json::Value>(&text) {
                    Ok(parsed) => {
                        ...
                        let sev_match = severity.to_lowercase() == rubric.expected_severity.to_lowercase();
                        eprintln!("{}ms sev={} terms={}/{}", latency, severity, terms_found, rubric.key_terms.len());

                        EvalRow {
                            fixture: name.clone(), category: rubric.category.clone(),
                            model: model.clone(), provider: provider.clone(), latency_ms: latency,
                            severity, expected_severity: rubric.expected_severity.clone(),
                            severity_match: sev_match,
                            key_terms_found: terms_found, key_terms_total: rubric.key_terms.len(),
                            component_match: comp_match, schema_valid: true, error: String::new(),
                        }
                    }
                    Err(e) => {
                        eprintln!("{}ms PARSE ERROR", latency);
                        EvalRow {
                            fixture: name.clone(), category: rubric.category.clone(),
                            model: model.clone(), provider: provider.clone(), latency_ms: latency,
                            ...
                            component_match: false, schema_valid: false, error: format!("parse: {e}"),
                        }
                    }
                }
            }
            Err(e) => {
                ...
                EvalRow {
                    ...
                    severity: String::new(), expected_severity: rubric.expected_severity.clone(),
                    severity_match: false, key_terms_found: 0, key_terms_total: rubric.key_terms.len(),
                    component_match: false, schema_valid: false, error: e.chars().take(200).collect(),
                }
            }
        };
